...
def make_datasets(config):
    ...
    if type == "KITTI_Odometry":
        ...
        # Handle image_size: None means use original resolution
        image_size = config.get("image_size", (192, 640))
        ...
        train_dataset = Kitti360Dataset(
            ...
            depth_source=config.get("depth_source", "lidar"),
            depth_da3_path=config.get("depth_da3_path", None),
            depth_da3_scale=config.get("depth_da3_scale", 256.0),
            ...
            online_depth_generation=config.get("online_depth_generation", False),
        )
        test_dataset = Kitti360Dataset(
            ...
            dilation=config.get("dilation", 1),
            is_preprocessed=config.get("is_preprocessed", False),
            load_kitti_360_segmentation_gt=False,  # Disabled: data_2d_semantics/semantic folder not available in preprocessed data
            constrain_to_datapoints=config.get("constrain_to_datapoints", False),
            additional_random_front_offset=config.get("additional_random_front_offset", False),
            online_depth_generation=config.get("online_depth_generation", False),
        )
