...
S4C with DA3 Adaptive Sampling (Monodepth2 encoder restored)
"""
...
from utils.projection_operations import invert_pose
...
class BTSNet(torch.nn.Module):
    def __init__(self, conf):
        ...
        # Adaptive Sampling Configuration (DA3-guided)
        self.adaptive_sampling = conf.get("adaptive_sampling", False)
        self.n_surface_samples = conf.get("n_surface_samples", 32)
        self.n_global_samples = conf.get("n_global_samples", 32)
        self.absrel_prior = conf.get("absrel_prior", 0.10)
        self.min_thickness = conf.get("min_thickness", 0.5)
        self.da3_depths = None

        # Original Monodepth2 encoder
        ...
        elif self.segmentation_mode in ('panoptic_deeplab', 'mask2former', 'm2f'):
    ...
    def encode(self, data, images, Ks, poses_c2w, ids_encoder=None, ids_render=None, images_alt=None, combine_ids=None):
        """
        Encode images using Monodepth2 backbone.
        Also extracts DA3 depths from data if available (for adaptive sampling).
        """
        # Extract DA3 depths if available
        if "depths" in data and len(data["depths"]) > 0:
            self.da3_depths = torch.stack(data["depths"], dim=1)  # (n, nv, 1, h, w)
        else:
            self.da3_depths = None

        poses_w2c = invert_pose(poses_c2w)
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, only_density=False, predict_segmentation=False):
        ...
        with profiler.record_function("model_inference"):
            ...
            if self.sample_color:
                ...
                rgb, invalid_colors = self.sample_colors(xyz)
            else:
                sigma = mlp_output[..., :1]
                sigma = F.relu(sigma)
            ...
            if not only_density:
                _, _, _, c = rgb.shape
                rgb = rgb.permute(0, 2, 1, 3).reshape(n, n_pts, nv * c)
                invalid_colors = invalid_colors.permute(0, 2, 1, 3).reshape(n, n_pts, nv)

                invalid = invalid_colors | invalid_features
