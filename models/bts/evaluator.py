...
class BTSWrapper(nn.Module):
    ...
    def forward(self, data):
        ...
        # 采样光线时同时获取对应的 DA3 depths
        sample_result = self.sampler.sample(images * .5 + .5, poses, projs, da3_depths=da3_depths)
        ...
        if hasattr(self.renderer.net, 'adaptive_sampling') and self.renderer.net.adaptive_sampling and all_da3_depths is not None:
            adaptive_config = {
                "n_surface": getattr(self.renderer.net, 'n_surface_samples', 16),
                "n_global": getattr(self.renderer.net, 'n_global_samples', 8),
                "absrel_prior": getattr(self.renderer.net, 'absrel_prior', 0.12),
                "min_thickness": getattr(self.renderer.net, 'min_thickness', 0.5),
            }
        ...
        # ============================================================
        # 渲染时使用 DA3 自适应采样 (与训练保持一致)
        # ============================================================
        render_dict = self.renderer(
            all_rays, 
            want_weights=True, 
            want_alphas=True,
            da3_depths=all_da3_depths,
            adaptive_sampling_config=adaptive_config
        )
