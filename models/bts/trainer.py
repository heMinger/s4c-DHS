...
from models.da3_depth_generator import get_da3_generator, DA3DepthGenerator
...
from utils.projection_operations import distance_to_z, invert_pose
...
class BTSWrapper(nn.Module):
    def __init__(self, renderer, config, eval_nvs=False) -> None:
        ...
        self.online_depth_generation = config.get("online_depth_generation", False)
        self.da3_checkpoint_path = config.get("da3_checkpoint_path", 
            "/home/lmh/s4c-adaptive-sampling1/DA3/ckpt/DA3NESTED-GIANT-LARGE")
        self.da3_conf_percentile = config.get("da3_conf_percentile", 20.0)
        self._da3_generator = None
        
    def get_da3_generator(self):
        """Lazily initialize DA3 depth generator."""
        if self._da3_generator is None and self.online_depth_generation:
            self._da3_generator = get_da3_generator(
                checkpoint_path=self.da3_checkpoint_path,
                device=next(self.renderer.parameters()).device,
                conf_percentile=self.da3_conf_percentile
            )
        return self._da3_generator
    ...
    def forward(self, data):
        ...
        poses = torch.stack(data["poses"], dim=1).float()                   # n, v, 4, 4 w2c - ensure float32
        projs = torch.stack(data["projs"], dim=1).float()                   # n, v, 4, 4 (-1, 1) - ensure float32
        ...
        to_base_pose = invert_pose(poses[:, :1, :, :])
        ...
        with profiler.record_function("trainer_encode-grid"):
            ...
            # 传递 data 字典以便 encode 可以访问 DA3 depths
            self.renderer.net.encode(data, images, projs, poses, ids_encoder=ids_encoder, ids_render=ids_render, images_alt=images_ip, combine_ids=combine_ids)
        
        # 获取 DA3 depths 用于自适应采样
        da3_depths = None
        da3_confs = None
        low_conf_mask = None
        ...
        if self.online_depth_generation:
            # Online DA3 depth generation
            da3_gen = self.get_da3_generator()
            if da3_gen is not None:
                with torch.no_grad():
                    # Reshape images for batch processing: (n, v, c, h, w) -> (n*v, c, h, w)
                    images_flat = images.reshape(n * v, c, h, w)
                    
                    # Create mask for perspective cameras (cam_idx 0,1 are perspective, 2,3 are fisheye)
                    # Assuming camera order: [cam_00, cam_01, cam_02, cam_03] 
                    # cam_00, cam_01 = perspective; cam_02, cam_03 = fisheye
                    is_perspective = torch.zeros(n * v, dtype=torch.bool, device=device)
                    for batch_idx in range(n):
                        for view_idx in range(v):
                            flat_idx = batch_idx * v + view_idx
                            # Check if this is a perspective camera (view 0,1 are typically perspective)
                            # This depends on the dataset's camera ordering
                            is_perspective[flat_idx] = view_idx < 2 or v <= 2
                    
                    # Generate depth maps
                    depth_flat, conf_flat, low_conf_flat = da3_gen.generate_for_batch(
                        images_flat, is_perspective
                    )
                    
                    # Reshape back to (n, v, h, w)
                    da3_depths = depth_flat.reshape(n, v, h, w).unsqueeze(2)  # (n, v, 1, h, w)
                    da3_confs = conf_flat.reshape(n, v, h, w).unsqueeze(2)   # (n, v, 1, h, w)
                    low_conf_mask = low_conf_flat.reshape(n, v, h, w)        # (n, v, h, w)
        elif "depths" in data and len(data["depths"]) > 0:
            # Offline depth loading
            da3_depths = torch.stack(data["depths"], dim=1)  # (n, v, 1, h, w)
            # For offline mode, no confidence filtering (all high confidence)
            low_conf_mask = None

        sampler = self.train_sampler if self.training else self.val_sampler
        
        # 获取用于 loss 计算的 DA3 depths 和置信度 (仅在训练时使用)
        da3_depths_loss = None
        low_conf_mask_loss = None
        if da3_depths is not None:
            da3_depths_loss = da3_depths[:, ids_loss]  # (n, len(ids_loss), 1, h, w)
            if low_conf_mask is not None:
                low_conf_mask_loss = low_conf_mask[:, ids_loss]  # (n, len(ids_loss), h, w)

        with profiler.record_function("trainer_sample-rays"):
            sample_result = sampler.sample(images_ip[:, ids_loss], poses[:, ids_loss], projs[:, ids_loss], 
                                           da3_depths=da3_depths_loss, low_conf_mask=low_conf_mask_loss)
            if len(sample_result) == 4:
                all_rays, all_rgb_gt, all_da3_depths, all_low_conf_mask = sample_result
            elif len(sample_result) == 3:
                all_rays, all_rgb_gt, all_da3_depths = sample_result
                all_low_conf_mask = None
            else:
                all_rays, all_rgb_gt = sample_result
                all_da3_depths = None
                all_low_conf_mask = None

        with profiler.record_function("trainer_sample_segmentation-rays"):
            # 获取用于分割的 DA3 depths 和置信度
            da3_depths_seg = None
            low_conf_mask_seg = None
            if da3_depths is not None and n_segs > 0:
                da3_depths_seg = da3_depths[:, :n_segs]
                if low_conf_mask is not None:
                    low_conf_mask_seg = low_conf_mask[:, :n_segs]
            seg_result = sampler.sample(
                images_ip[:,:n_segs], poses[:, :n_segs], projs[:, :n_segs], segs[:, :n_segs], sample_segs=True, 
                da3_depths=da3_depths_seg, low_conf_mask=low_conf_mask_seg)
            if len(seg_result) == 6:
                segmentation_rays, seg_rgb_gt, segmentation_gt, patch_to_image, seg_da3_depths, seg_low_conf_mask = seg_result
            elif len(seg_result) == 5:
                segmentation_rays, seg_rgb_gt, segmentation_gt, patch_to_image, seg_da3_depths = seg_result
                seg_low_conf_mask = None
            else:
                segmentation_rays, seg_rgb_gt, segmentation_gt, patch_to_image = seg_result
                seg_da3_depths = None
                seg_low_conf_mask = None
        
        # 构建自适应采样配置
        adaptive_config = None
        if self.renderer.net.adaptive_sampling and da3_depths is not None:
            adaptive_config = {
                "n_surface": self.renderer.net.n_surface_samples,
                "n_global": self.renderer.net.n_global_samples,
                "absrel_prior": self.renderer.net.absrel_prior,
                "min_thickness": self.renderer.net.min_thickness,
            }

        data["patch_to_image"] = patch_to_image.to(device) if isinstance(patch_to_image, torch.Tensor) else patch_to_image
        data["fine"] = []
        data["coarse"] = []
        data["segmentation"] = []
        ...
        if self.prediction_mode == "multiscale":
            for scale in self.renderer.net.encoder.scales:
                ...
                render_dict = self.renderer(all_rays, want_weights=True, want_alphas=True, want_rgb_samps=True,
                                            da3_depths=all_da3_depths, adaptive_sampling_config=adaptive_config,
                                            low_conf_mask=all_low_conf_mask)
                if scale != 0 and using_fine:
            ...
            with profiler.record_function("trainer_segmentation_render"):
                render_segmentation_dict = self.renderer(segmentation_rays, want_weights=True, want_alphas=True,
                                                          ...
                                                          da3_depths=seg_da3_depths, adaptive_sampling_config=adaptive_config,
                                                          low_conf_mask=seg_low_conf_mask)
        ...
        if self.training is False:
            ...
            # normal metrics (DA3 depth comparison)
            if len(data["depths"]) > 0:
                data.update(self.compute_depth_metrics(data))
            
            # LiDAR depth metrics (always call - returns NaN if no valid data)
            data.update(self.compute_lidar_depth_metrics(data))
            ...
            cam_incl_adjust = torch.tensor(
                ...
                dtype=torch.float32, device=device).view(1, 4, 4)
    ...
    def compute_segmentation_metrics(self, data):
        ...
        # Handle empty segs_kitti_gt (when load_kitti_360_segmentation_gt=False)
        has_kitti_gt = len(data["segs_kitti_gt"]) > 0 and data["segs_kitti_gt"][0].numel() > 0
        if has_kitti_gt:
        ...
        device = segs_pred.device

        # standard accuracy
        acc = segs_pred.eq(segs_gt).float().mean()

        # front and side separation
        acc_side = torch.tensor(0, dtype=torch.float, device=device)
        ...
        # denosing metrics (skip if no KITTI GT available)
        if has_kitti_gt and n_kitti > 0:
            ...
            acc_kitti_vs_gt = torch.tensor(0.0, device=device)
            acc_kitti_vs_pred = torch.tensor(0.0, device=device)
    ...
    def compute_lidar_depth_metrics(self, data):
        """
        Compute depth metrics using LiDAR ground truth (only at pixels with valid LiDAR).
        Uses frame 0 for evaluation.
        Always returns metrics (with NaN if no valid data).
        """
        device = data["fine"][0]["depth"].device
        nan_tensor = torch.tensor([float('nan')], device=device)
        
        # Check if lidar_depths is available
        if "lidar_depths" not in data or len(data["lidar_depths"]) == 0:
            return {
                "a1_lidar": nan_tensor,
                "abs_rel_lidar": nan_tensor,
            }
        
        # LiDAR depth for frame 0
        lidar_depth_gt = data["lidar_depths"][0]  # (1, 1, H, W)
        depth_pred = data["fine"][0]["depth"][:, :1]  # (1, 1, h, w)
        
        depth_pred = F.interpolate(depth_pred, lidar_depth_gt.shape[-2:])
        depth_pred = torch.clamp(depth_pred, 1e-3, 80)
        
        # Only evaluate at valid LiDAR pixels
        mask = lidar_depth_gt > 0
        ...
        lidar_gt = lidar_depth_gt[mask]
        pred = depth_pred[mask]
        
        thresh = torch.maximum((lidar_gt / pred), (pred / lidar_gt))
        a1_lidar = (thresh < 1.25).to(torch.float).mean()
        
        abs_rel_lidar = torch.mean(torch.abs(lidar_gt - pred) / lidar_gt)
        
        metrics_dict = {
            "a1_lidar": a1_lidar.view(1),
            "abs_rel_lidar": abs_rel_lidar.view(1),
        }
        return metrics_dict
...
def get_dataflow(config, logger=None):
    ...
    # ============================================================
    # Enable DA3 depth loading for training (required for adaptive sampling)
    # Enable LiDAR depth loading for evaluation metrics
    # ============================================================
    train_dataset.return_depth = True
    train_dataset.return_lidar_depth = True
    ...
    test_dataset.return_lidar_depth = True  # Enable LiDAR depth for evaluation
    ...
    vis_dataset.return_lidar_depth = True  # Enable LiDAR depth for visualization
...
def get_metrics(config, device):
    ...
    # LiDAR depth metrics (compared with LiDAR GT)
    names += ["a1_lidar", "abs_rel_lidar"]
...
def visualize(engine: Engine, logger: TensorboardLogger, step: int, tag: str):
    ...
    if segmentation_kitti_gt is not None:
    ...
    segmentation_kitti_gt_grid = make_grid(segmentation_kitti_gt, nrow=n_row_seg) if segmentation_kitti_gt is not None else None
    ...
    if segmentation_kitti_gt_grid is not None:
